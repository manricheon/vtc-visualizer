...
- **데이터 모델**: `addDataset`, `allRows`(병합), `columns`, `numericColumns`, `uniqueVals`, `matchFilter`(단일 필터), `applyFilters`(제외 모드만 행 제거), `isDimmed`/`isMutedRow`
  ...
  **두 모드 다 "조건에 맞는 쪽"을 남긴다** — `=`와 `≠` 중 어느 쪽이 흐려지는지 헷갈리기 쉬우므로(실제로 반대로 이해한 적 있음)

...
- **보고서 출력**(v0.13): `cfg.caption`(카드 하단 `.plot-caption`, `applyCaption()`으로 부분 갱신) →
...
- **계산 컬럼**: `state.derived[]`(세션 저장, 입력 계약 아님 — 뷰 계층) → `applyDerived()`가 로드/변경 시 각 행에 파생 값 주입
  ...
  **정의 순서가 곧 계산 순서**(체인 의존성) — UI에 ↑/↓와 편집(`derivedEditIdx`)이 있다. 설명 문구는 `derivedDesc()`.
  프리셋은 `presetDerivedFor()`로 그 차트가 쓰는 정의를 의존성까지 모아 `_derived`에 담고, `applyPreset()`이 없는 정의를 복원한다. UI는 `renderDerived()`(데이터 카드의 `#derivedPanel`)

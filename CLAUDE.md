...
## 파일 구조 (소스·문서는 이 **11개**가 전부 — 파일을 늘리지 말 것. 예외: `assets/guide/`는 가이드 예시 이미지 전용 폴더)

> **10 → 11이 된 이유**(v0.54): `CHANGELOG.md`를 갈랐다. README의 68%(493줄)가 변경 이력이라
> "이 도구가 무엇인지" 읽으러 온 사람이 릴리스 노트 열다섯 개를 지나야 했다. 이력은 사라지지 않고 옮겨졌을 뿐이고,
> README는 244줄로 줄어 읽을 수 있는 문서가 됐다. **다음에 파일을 더하려면 이 정도 근거를 대고 이 문장도 함께 고칠 것.**
...
| `README.md` | 사용자 문서 (한국어): 5분 시작, 기능, 입력 계약, 에이전트용 변환 요청문. **변경 이력은 여기 두지 않는다** — 최근 5개 요약만 |
| `CHANGELOG.md` | 전체 변경 이력(영문 제목 + 한국어 병기). 릴리스마다 여기에 항목을 더한다 |
...
| `assets/guide/*.png` | 가이드 레시피별 예시 캡처(r1~r20 — 파일 이름의 번호가 곧 레시피 번호다(v0.54), 헤드리스 Chrome으로 `.plot` 요소만 2x 크롭 — r12만 `#analysisCard`, r13은 주석 도구 결과물) — **레시피 설정이나 차트 렌더링이 바뀔 때만** 재캡처. 화면 UI가 바뀌어도 이 그림들은 플롯만 담으므로 대개 그대로다(v0.30·v0.45에서 확인: 팔레트·`CHROME`·`faceColor`·`buildLayout` 변경 0줄이면 다시 찍을 이유가 없다.
  v0.45에서 실제로 r1을 다시 찍어 대조했고 같은 그림이었다 — **의심되면 한두 장만 다시 찍어 대조**하고, 같으면 커밋하지 말 것: 바이너리 diff만 남는다). 재생성 스크립트는 커밋하지 않음 |
...
- **보이는 것만 그리기**(v0.65): 전체를 갈아치우는 입구는 **`rerenderAllPlots()` 하나**다(테마·팔레트·언어·표 조작·계산 컬럼 commit).
  ...
  낡은 그림이 파일로 나가는 것이 이 변경의 최악 실패다. restyle로 색만 바꾸는 길은 **가지 않았다**:
  팔레트가 trace 마커·선·밴드·주석에 박혀 있어 한 곳만 새면 반쪽 테마가 된다(조용히 틀리는 종류).
  실측: 12장 × 20만 행 테마 전환 931 → 303ms(10장 미룸).
- **필터·탐색**(v0.64): `between`은 값이 둘(`value`=하한, `v2`=상한, **경계 포함**, 한쪽만 넣으면 그 방향만 — 축 범위와 같은 규약).
  `filterActive`가 `v2`도 본다 — 안 보면 상한만 넣은 필터가 "미완성"으로 무시된다.
  결합은 **차트 단위 하나**(`filterJoin: and|or`, 기본 and — 옛 세션은 `defaultChart` 병합으로 and).
  **미완성 필터는 결합에서 빠진다**: OR에서 미완성이 true를 주면 필터 하나만 지워도 전부 통과한다.
  흐리게(`isDimmed`)도 같은 결합을 "밝게 남는" 판정으로 편다(드모르간 — exclude와 같은 뜻이 되게).
  필터별 OR 그룹은 **안 만든다**(쿼리 빌더가 된다 — 차트 복제가 그 답).
  표 값 셀 클릭 → `showCellFilterMenu` — 대상은 **도크가 보고 있는 차트**(`dockChartId`, 없으면 첫 차트).
  셀 리스너는 `renderTable` 안에서 렌더마다 다시 붙는다(innerHTML 통짜 재생성과 같은 수명). 양쪽 손잡이 슬라이더는
  겹친 `input[type=range]` 둘 — 트랙은 `pointer-events: none`, **손잡이만** 이벤트를 받아야 아래 슬라이더도 잡힌다.
- **컬럼 이름을 담는 차트 필드는 두 종류**(v0.63.1): 스칼라 `COL_REF_FIELDS`와 **목록 `COL_REF_LISTS`**(지금은 `matrixCols`).
  훑는 코드는 **`forEachColRef` / `renameColRefs` / `dropColRefs` 셋만** 쓴다 — 개명(`renameDerived`)·사용처(`colUsage`)·
  참조 정리(`clearDerivedRefs`)·데이터셋 전환(`switchDataset`)·프리셋 번들(`chartRefCols`)이 각자 목록을 돌던 것을 모은 것이다.
  **새 필드를 더하면 둘 중 하나에 등록할 것** — v0.59에서 행렬을 배열로 담고 등록하지 않아,
  같은 이름 컬럼이 든 파일을 열면 **행렬이 아무 말 없이 파일의 다른 값을 그렸다**(v0.53이 막으려던 사고와 같은 종류다).
- **순위와 구성비**(v0.63): 옵션 셋이 표현 둘을 연다 — **새 유형을 만들지 않았다**.
  `xReverse`/`yReverse`는 `axis()`의 마지막 인자다: **범위를 직접 준 축은 그 범위를 뒤집고, 자동이면 `autorange: 'reversed'`**
  (자동인데 `range`만 뒤집으면 Plotly가 다시 정방향으로 계산한다). **`uirevision` 키에 넣을 것** —
  안 넣으면 줌 유지 규칙이 옛 범위를 붙들어 체크를 켜도 그림이 그대로다(실제로 그랬다).
  눈금 `int1`은 `tickformat: 'd'` + **`dtick: 1`**이다. 기존 `'0'`은 표기만 바꾸며 그대로 둔다 — 0~16000 축에 `dtick: 1`을 걸면 눈금이 만 육천 개다.
  `barMode: 'stack100'`은 Plotly에 없는 모드라 **`barmode: 'stack'` + `barnorm: 'percent'`**로 편다(`barModeLayout(cfg)` 하나가 정한다).
  **`barnorm`은 그리는 것만 정규화한다** — 막대 값 표시는 원값이라 100% 막대에 합이 100이 아닌 숫자가 찍힌다.
  `share100Text(cfg, traces)`가 같은 X의 합으로 나눠 다시 쓰고(facet 포함), 값 축 제목에 `(%)`를 붙인다.
  범프 차트는 이 셋 + `순위` 계산 컬럼 + 선으로 서므로 **유형이 아니라 레시피(㉒)**다.
- **값이 줄 서는 순서**(v0.62): **`compareVals(a, b)` 하나가 정한다** — 선을 잇는 순서(`buildFacetTraces`·보조 Y축),
  `uniqueVals`(히트맵·덤벨 축, 분할 값, 그룹·모양 순서, 필터 선택지, 기준행 목록), 막대의 `라벨 순`이 전부 여기를 부른다.
  같은 비교식이 네 벌로 흩어져 있어 **같은 컬럼이 선 차트와 히트맵에서 다른 순서**로 섰다.
  규칙: 숫자는 크기순, 문자열은 **자연 정렬**(`Intl.Collator`, `numeric: true` — 사전순이면 `v2`가 `v10` 뒤로 간다), 섞이면 숫자 먼저, 빈 값은 뒤.
  **새로 정렬하는 자리를 만들면 여기를 부를 것.** 막대 `라벨 순`은 Plotly의 `category ascending` 대신 **`categoryarray`를 직접 준다**.
  덤벨도 카테고리 축에 `categoryarray`를 준다 — 안 주면 trace에 나온 순서로 쌓여 어긋난다(연결선은 그룹이 둘 이상인 카테고리에만 생긴다).
  **순수 산점도·막대는 정렬하지 않는다.** 같은 X에 점이 여럿이면 `dupXCount(cfg)`가 세어 한 줄로 알린다 —
  지그재그는 "반복 측정이 있다"는 정직한 신호라 감추지 않고 빠져나갈 길(집계)을 함께 적는다.
- **폭·범례 자리**(v0.61): 카드 폭은 `cardWidth: full|twothird|half|third`이고 클래스는 `widthClass(cfg)` 하나가 정한다
  (**`half`는 v0.7부터 세션에 있는 이름**이라 그대로 받고 새 이름은 `w-half`). CSS는 `flex: 0 1 calc(…)` —
  **`flex-grow: 0`이 핵심**이다: 늘어나게 두면 줄에 혼자 남은 카드가 100%로 부풀어 고른 폭이 이웃 수에 따라 달라진다.
  범례 자리는 `LEGEND_POS`(바깥 아홉) + `LEGEND_INSIDE`(안쪽 넷) + `LEGEND_ALIAS`(옛 `right`·`top`·`inside` → 새 이름).
  **`legendKey()`를 거치지 않고 `cfg.legendPos`를 직접 비교하지 말 것** — 옛 값이 들어오면 자리가 빈다.
  여백은 `legendRightFree`/`legendTopRoom`/`legendBottomRoom` 셋이 정한다(같은 식이 네 곳에 흩어져 있었다).
  아래 자리는 **`yref: 'container'`**로 그림 상자 바닥에 붙인다 — 높이 비율로 내리면 짧은 카드에서 축 제목과 겹친다(실측).
- **설정 도크**(v0.60): 설정은 **카드 안이 아니라 페이지 왼쪽(`#dock`)에 하나**다. 카드 안에는 같은 UI를 두지 않는다(두 자리에 두면 결국 한쪽에만 기능이 붙는다).
  `buildCfgPanel(cfg, panel, nameSpan)`의 시그니처는 그대로고 도크가 자기 `.cfg`와 그 카드의 이름 span을 넘긴다. `refreshCfgPanels()`는 **도크 하나만** 다시 짓는다.
  **편집 대상은 하나**(`dockChartId`) — "지금 어느 차트를 고치는 중인가"를 **두 곳에서** 말한다(도크 제목 + `.chart-card.editing` 테두리). 한 곳만 두면 엉뚱한 차트를 고친다.
  **닫는 데는 두 가지가 있다**: `closeDock()`은 사람이 닫은 것이라 `'off'`로 기억하고, `dropDock()`은 대상이 사라진 것이라 기억하지 않는다
  (차트가 없는 잠깐 사이에 `'off'`가 찍히면 처음 여는 사람에게도 설정이 안 열린다 — 실제로 그랬다). 첫 차트에서는 저절로 열린다(v0.59까지 패널이 펼쳐진 채 나타났다).
  **카드를 다시 지으면(프리셋·유형 변경) 도크도 다시 짓는다** — `renderChartCard`가 `dockChartId`를 보고 `refreshCfgPanels()`를 예약한다(안 하면 프리셋 적용 뒤 패널이 옛 상태로 남는다).
  열고 닫을 때 **`resizePlots()`**(접힌 차트는 reject하므로 건너뛴다). 좁은 화면(≤860px)에서는 `position: fixed` 오버레이 + 배경 클릭/Escape,
  **Escape로 닫을 때만 포커스를 되돌린다**(바깥 클릭은 방금 누른 것을 뺏지 않는다 — `hidePopover(restore)`와 같은 판단).
  검사에서 패널을 찾을 때는 `#dock .cfg`이고 여는 길은 `_helpers.mjs`의 **`openCfg(page, idx)`** 하나다.
...
- **조판 내보내기**(v0.46): `showSheetPopover` → `exportSheet(opt)` — 고른 차트를 격자로 그려 PNG 한 장.
  **각 칸은 화면 크기 그대로 뽑고 배율만 준다**(`toImage({width: gd.clientWidth, height: gd.clientHeight, scale: cellW/gw*scale})`) —
  칸 폭에 맞춰 다시 그리면 글자 크기는 그대로인 채 그림만 작아져 **글자가 상대적으로 거대해지고 범례가 잘린다**(처음에 그렇게 만들었다가 조판 그림에서 바로 드러났다).
  칸 높이는 화면 비율을 따른다 — 억지로 같은 높이로 늘리면 칸마다 눈금 간격이 달라져 비교가 거짓말이 된다.
  판 전체 폭이 규격(mm/inch × dpi)이고 `sheetSize()`가 `exportSize()`와 같은 96 CSS dpi 규약을 쓴다.
- **보기 설정 일괄 적용**(v0.46): `LOOK_FIELDS` + `showLookCopyMenu` — 필터 복사(`showFilterCopyMenu`)와 **같은 자리·같은 모양**이다.
  **컬럼을 가리키는 필드는 절대 넣지 말 것**(x·y·group·filters·계산 컬럼): 차트마다 다른 데이터를 그리므로 그것까지 복사하면 그리던 그림이 통째로 바뀐다.
  여러 장을 한 번에 바꾸므로 **되돌리기 토스트가 필수**다(직전 값을 필드별로 담아 둔다).
- **카드 단축키**(v0.46): 전역 keydown에서 **Alt + 화살표만** 본다 — 글자 키는 입력칸과 충돌하고 화살표 단독은 스크롤·선택을 빼앗는다.
  포커스가 그 카드 안에 있을 때만 동작한다(`e.target.closest('.chart-card')`). **옮긴 뒤 포커스를 되돌린다** — DOM에서 노드를 옮기면 브라우저가 포커스를 잃어 연달아 못 옮긴다.
- **데이터 카드 접기**(v0.45): `DATA_FOLDED`(키 `vtc-visualizer:datafold`) + `applyDataFold()` — 245 → 52px, 첫 차트가 335 → 142px에서 시작한다.
  차트 카드 접기와 같은 뜻이지만 **세션이 아니라 브라우저 기본값**이다(페이지 하나의 상태라 차트별이 아니다 — `ACCORDION`·`SIMPLE`과 같은 급).
  **데이터가 없으면 접지 않는다** — 처음 온 사람에게 드롭존이 사라지면 시작할 곳이 없다. 접혀 있어도 무엇이 들어 있는지는 `#dataSummary` 한 줄로 보인다.
...
- **전역 언두**(v0.69): `undoCapture`는 **`save()` 디바운스 본문**에서만 불린다 — 한 칸 = 손을 멈춘 순간.
  스냅샷에 **행 데이터는 없다**(설정 + 큐레이션 rowKey만, KB 단위, 상한 50) — 데이터는 되돌릴 대상이 아니다(파일이 진실).
  `undoApply` 중의 save가 스택을 다시 쌓으면 안 되므로 `undoApplying` 가드를 디바운스가 끝난 뒤(600ms) 푼다.
  **입력칸 안의 Ctrl+Z는 절대 가로채지 않는다**(브라우저의 글자 되돌리기가 먼저). 새 변경은 redo를 비운다 — 미래는 하나다.
  토스트 undo(v0.49)는 그대로 산다 — 이건 그 위의 안전망이지 대체가 아니다.
...
- **i18n**: UI는 KO/EN 이중 언어(`I18N` 사전 + `t()`/`tf()`, 토글 = `#btnLangToggle`, 저장 키 `vtc-visualizer:lang`).
  ...
  **릴리스 전에는 `visualizer-local/vtc-tests/preflight.sh`를 돌린다** — 버전 3곳 일치·오프라인 동기화·i18n 짝·문서 대칭·그림 링크·파일 수를 검사한다(`run.sh`가 먼저 부른다).
  v0.54에서 **문서 검사**를 더했다 — 상대 링크·앵커가 실제로 있는가, 가이드 설정 블록(`> `)이 `EXTRA_CSVS`·`example.csv`에 **있는 파일만** 가리키는가,
  레시피 번호(①~⑳)가 파일 순서와 맞고 그림 이름이 그 번호인가, `© mrc`가 각 문서 끝(접힌 `<details>` 밖)인가, `CHANGELOG.md`에 이번 버전 항목이 있는가.
  따라 할 수 없는 레시피(`models.csv`)가 오래 남아 있던 것이 계기다 — 문서는 아무 소리 없이 틀린다.
...
- **팝오버 크기·위치**(v0.50): `#ptPopover`에 `max-width: min(92vw,420px)` · `max-height: min(78vh,620px)` · `overflow:auto`.
  `placePopover`는 **실제 크기로 클램프**한다(상수 230×140을 쓰다 조판 팝오버가 화면 밖으로 나갔다) —
  크기를 재려면 먼저 보여야 하므로 `show`를 켜고 화면 밖(`left:-9999px`)에 둔 뒤 좌표를 정한다.
  목록이 길어질 수 있는 팝오버는 `.pp-list`(max-height + 스크롤)를 쓴다 — 팝오버 전체가 길어지면 버튼이 화면 밖으로 밀린다.
...
- **산점도 행렬**(v0.59): 유형 `matrix`, 판정은 **`isMatrixType()` 하나**(유형 이름을 여기저기 나열하면 v0.32처럼 조용히 빈 화면이 된다).
  x·y가 아니라 **컬럼 목록**(`matrixCols`, 고른 순서 = 격자 순서)으로 그리므로 `renderPlot`의 빈 화면 판정도 이 유형만 따로 본다.
  행은 **`chartBaseRows(cfg)`** — `chartRows`를 쓰면 x/y 값 유무로 걸러져 이 유형에서는 뜻이 없다.
  **한계는 자르되 말한다**: 컬럼은 `MATRIX_MAX_COLS`(8, 그 이상은 64칸이라 안 읽힌다), 점은 `cfg.matrixMax`(칸당 5,000)에서 자르고
  `matrixNote(cfg)`가 무엇을 뺐는지 설정 패널에 적는다. 표본은 **결정적 stride**다(무작위면 볼 때마다 다른 그림이다).
  **대각선에 히스토그램은 없다** — splom의 대각선은 자기 자신 대 자기 자신(직선 하나)이라 기본은 비운다. 라벨도 그렇게 적혀 있어야 한다.
  **SVG로 내보내도 점은 래스터다**(splom은 SVG 경로가 없다 — 점 수와 무관, 실측). 막지 않고 패널·내보내기에서 말한다.
  레이아웃은 splom이 축을 스스로 만들므로 **`xaxis`/`yaxis`를 주면 첫 칸만 덮어써 격자가 어긋난다** — 축마다(`xaxisN`) 격자 모양만 얹는다.
- **초기화·복원이 남기는 것**(v0.58): 전체 초기화는 `state`만이 아니라 **`datasetsMeta`와 저장 꺼짐 표시(`STALE_KEY`)까지** 비운다 —
  안 비우면 방금 지운 파일을 "다시 넣으라"고 조르고, 다음 실행에 "저장 공간이 꽉 찼었다"고 말한다.
  `hydrateData()`는 **`await` 뒤에 다시 판정한다**: 읽어 오는 사이에 사용자가 같은 파일을 넣었으면 같은 이름이 두 벌이 되고
  행 정체성·필터·`_source`가 전부 어긋난다. 그때는 **방금 넣은 쪽이 이긴다**(복원본은 지난번 내용이다).
  `idbLoad()`는 **없으면 `null`** — `idbTx`가 결과 없음을 `true`로 주므로 그대로 흘리면 "담긴 것이 있다"로 읽힌다.
- **행은 IndexedDB에**(v0.57): `idbOpen`/`idbSave`/`idbLoad`/`idbClear` — 스토어 하나, 레코드 하나(`'datasets'`). 라이브러리 없이 60줄이다.
  **없으면 없는 것처럼 군다**: 열기 실패·부재(사생활 모드)면 `idbOpen`이 `null`을 주고 `saveData()`가 v0.56의 localStorage 자리로 돌아간다. 둘 다 막히면 그때만 `저장 꺼짐`.
  **두 자리에 같이 두지 않는다** — IDB에 담으면 `DATA_KEY`를 지운다(두 벌이면 다음에 열 때 어느 쪽이 진짜인지 모른다). v0.56 세션은 읽어서 다음 저장에 옮긴다.
  복원은 비동기라 **설정은 동기로 먼저** 서고 행은 `hydrateData()`가 뒤따라 넣는다. **`datasetsMeta`에 있는 이름만 채운다** —
  다른 세션이 남긴 데이터가 지금 화면으로 새어 들어오면 안 된다(검사 스위트가 같은 origin을 공유한다).
  순서는 `hydrateData().then(tryServerAutoload)` — 서버 파일이 **마지막 말**을 해야 폴더의 최신 내용이 이긴다.
  실측(20만 행): 쓰기 67ms · 읽기 116ms · 실제 새로고침에서 20만 행 복원.
- **저장 계층**(v0.56): **설정과 데이터는 다른 키다** — `STORAGE_KEY`(charts·derived·hiddenCols·nextId·`datasetsMeta`, 수 KB)와 `DATA_KEY`(행).
  `save()`는 **설정을 먼저 쓴다**: 데이터가 크다는 이유로 차트 설정까지 잃는 것이 v0.55까지의 동작이었다(20만 행 = 15.1MB, 한도 5MB).
  **서버로 읽은 데이터셋(`ds.server`)은 담지 않는다** — 다음 실행의 자동 로드가 다시 읽는다. 사용자가 같은 파일을 직접 넣으면 플래그가 내려간다.
  **데이터 쓰기가 실패하면 옛 데이터를 지운다** — 남기면 다음에 열 때 지금과 다른 행으로 조용히 복원된다(없는 것은 유령 칩이 말하지만 옛 데이터는 아무 말도 안 한다).
  **`onDataChanged`는 `state.datasets.length === 0`이면 아무것도 정리하지 않는다** — 데이터가 사라진 것이 아니라 아직 안 온 것이다.
  가드가 없으면 `_source` 참조 정리와 `switchDataset(c, '')`가 **살리려던 축·필터를 지운다**(이 릴리스의 본체다).
  유령 칩은 `datasetsMeta`에서 나오므로 **`save()`마다 다시 만들고 `removeDataset`에서 지운다** — 낡으면 내가 지운 파일을 다시 넣으라고 조른다.
...
- **이름 충돌**(v0.53): 예약어와 계산 컬럼 두 가지를 `addDataset` **진입부**에서 처리한다(`renameReserved` · `freeDerivedName` + `renameDerived`).
  ...
  **규칙은 결정적이어야 한다** — 같은 파일을 다시 넣을 때 이름이 달라지면 `rowKeyCols`가 바뀌어 제외·흐리게·마커 앵커가 전부 어긋난다.
  `renameDerived(from,to)`가 옮기는 곳: 정의 이름 · 다른 정의의 입력(`renameDerivedInputs` — `derivedInputs`의 쓰기 짝, **새 kind를 더하면 둘 다 고칠 것**) ·
  `COL_REF_FIELDS` · 필터 · **마커·기준선의 `xc`/`yc`**(안 옮기면 앵커가 `stale:'axis'`가 되어 주석이 사라진다) · `hiddenCols` · `tableState.sortCol` ·
  **`derivedApplied`와 이미 뿌려진 값**(장부가 옛 이름을 들고 있으면 다음 `applyDerived`가 막 들어온 **파일의 값**을 지운다 — 실제로 그랬다).
  **저장된 프리셋은 건드리지 않는다**(다른 데이터에도 쓰는 저장물이고 `applyPreset`이 없는 정의를 복원한다).
  컬럼 이름을 담는 차트 필드는 **`COL_REF_FIELDS` 하나**다 — 예전에는 같은 목록이 세 벌이었고 그중 하나에 `y2`·`size`·`errCol`이 빠져 있었다.
- **조용한 실패**(v0.53): 자동 로드·감시는 실패를 모아 `reportLoadFailures(fails, extra)`로 **한 줄**(사유는 `보기` 팝오버).
  `/api/files`는 `{files, truncated}`를 주고 프런트는 배열도 받는다(옛 서버 방어). `/api/file`은 UTF-8 디코드가 안 되면 **400**(모지바케로 조용히 읽히는 것보다 낫다).
  축 후보에서 빠진 컬럼은 `nearNumericCols()`(표본 200개, 숫자 비율 ≥0.8, `dataVer` 캐시)로 사유를 낸다 — 진짜 범주형까지 나열하면 소음이다.
  **성공만 했으면 아무 말도 하지 않는다.**
...
- **가로 → 세로 녹이기**(v0.27): `meltRows(rows, idCols, valCols, varName, valName)` + `showMeltPopover()`.
  **행이 늘어나므로 계산 컬럼이 아니다** — 계산 컬럼은 행 안에서만 동작하고 행 증가는 행 정체성을 깬다.
  ...
  기본 추정은 `meltGuess`(v0.71) — ① 이름 줄기 묶음(run1·run2, acc_1k·acc_2k) ② 없으면 값 범위가 겹치는 가장 큰 묶음 ③ 그것도 없으면 숫자 전부. 줄기가 먼저다(단조 증가 측정값은 범위가 안 겹친다). 묶음은 값 집합을 줄이기만 하므로 옛 규칙보다 나빠질 수 없고, 틀리면 체크박스로 고친다.
...
- **붙이기 대화 상자는 본문 모달과 같은 규칙**(v0.50): 열면 포커스가 안으로, Tab을 상자 안에 가두고,
  Escape·버튼으로 닫으면 **연 것으로 되돌린다**. 단 **배경 클릭으로 닫을 때는 되돌리지 않는다**(`hidePopover(restore)`와 같은 판단 —
  방금 누른 것을 빼앗지 않는다). 좁은 화면 분기(860px)는 이 페이지에서 **머리글·목록·상자 폭**만 손댄다 — 캔버스는 `fitView`가 이미 맡는다.
- 서버는 `/annotate.html` **한 경로만** 준다(`visualizer.py`). 정적 파일 서버로 만들면 레포 전체가 읽힌다.
- **두 그림 나란히**(v0.46, `joinImage(im, dir)`): 붙이는 순간 **한 장의 이미지가 된다** — 레이어를 만들지 않는다(좌표·되돌리기·저장이 지금 그대로여야 한다).
  기존 그림은 (0,0)에 그대로 두고 오른쪽/아래로만 붙이므로 **이미 찍어 둔 마크가 어긋나지 않는다**(그래서 붙이기는 마크를 지우지 않는다).
  붙이는 쪽은 같은 높이(또는 폭)로 비율을 지켜 줄인다. 새 이미지를 넣으면 `showJoinDialog`가 오른쪽/아래/바꾸기/취소를 묻는다 —
  **바꾸기만 마크를 지우므로 확인은 그쪽에만** 붙는다. 취소·Escape는 아무 일도 하지 않는다.
- 안 넣은 것: 레이어 · 자유 곡선 · **작업 상태 저장**(저장을 넣으면 세션·포맷 호환이라는 새 계약이 생긴다).

...
| `assets/guide/*.png` | 가이드 레시피별 예시 캡처(r1~r13, 헤드리스 Chrome으로 `.plot` 요소만 2x 크롭 — r10만 `#analysisCard`, r13은 주석 도구 결과물) — **레시피 설정이나 차트 렌더링이 바뀔 때만** 재캡처. 화면 UI가 바뀌어도 이 그림들은 플롯만 담으므로 대개 그대로다(v0.30·v0.45에서 확인: 팔레트·`CHROME`·`faceColor`·`buildLayout` 변경 0줄이면 다시 찍을 이유가 없다.
